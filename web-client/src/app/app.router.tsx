 ...
 * @returns 
 */
const Layout = (props: {username: string, rights: string[], routes: Route[], onLogout: () => void}) => {
  return (
    <>
      <CustomNavbar {...props}/>
      <Outlet />
 ...
 * @returns 
 */
function AppRouter(props: {username: string, rights: string[], onLogout: () => void}) {
  let isAdmin = props.rights.filter((right) => /.*admin/.test(right.toLowerCase())).length !== 0;
