...
import org.mcgill.splendorapi.api.game.dto.CityBoardDto;
...
public class CityMapperStrategyTest {

  private CityMapperStrategy mapper = new CityMapperStrategy();

  @Test
  public void testIsStrategy() {
    Game game = Game.builder().type(GameType.CITIES_ORIENT).build();
  ...
  public void testMap() throws InvalidCardType {
    ...
    cities = Arrays.asList(new City[] {}); // populate with test data
    ...
    Deck.add(OL3C3);
    Deck.add(OL3C4);
    cityGameBoard = CityGameBoard.builder()
                                 .gems(gems)
                                 .nobles(List.of())
                                 .deck(Deck)
                                 .cities(cities)
    ...
    CityBoardDto dto = mapper.map(cityGameBoard, nobles);
    ...
    Assert.assertEquals(cityGameBoard.getGems(), dto.getAvailableGems());
    Assert.assertEquals(cityGameBoard.getNobles(), dto.getNobles());
    Assert.assertEquals(cityGameBoard.getCities(), dto.getCities());
  }
}
